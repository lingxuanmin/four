...
iFourth.zengpin = function() {
    ...
    $('.zengpin tr').hover(function() {
        clearTimeout(timer);

        var obj = $(this).find('.img img'),
            src = obj.attr('src-large'),
            pos = obj.position();

        timer = setTimeout(function() {
            if (pop.size()== 0) {
                pop = $('<img style="position:absolute; width:200px; height:200px; border:solid 1px #EEE;" />')
                    .css({
                        left: pos.left - 205,
                        top: pos.top - 105
                    })
...
iFourth.o2oPop = {
    init: function(callback) {
        ...
        // 弹窗入口
        $('.proinfo-o2o li:not(.item1) a,.proinfo-o2o li:not(.item1) i').live('click', function() {
...
iFourth.showQRCode = function() {
    var box = $('.proinfo-focus').eq(0),
        qr = box.children('.qrcode-main,.qrcode-main-mobile');

    if (qr.is(':visible')) {
        box.addClass('proinfo-focus-qrcode');

        if (box.children('dl:visible').size() < 3) {
            box.addClass('proinfo-focus-qrcode-short');
        }
        else {
            box.removeClass('proinfo-focus-qrcode-short');
        }
    }
    else {
        box.removeClass('proinfo-focus-qrcode proinfo-focus-qrcode-short');
    }

    // 下拉事件
    var btn = $('.qrcode-main-handle .dropdown');
    var qrcode = $('.qrcode-main-img');
    var isIE8 = $.browser.msie && $.browser.version <= 8;

    if (isIE8) {
        qrcode.find('i').css('opacity', 0);
    }
    ...
    btn.click(function(e) {
        ...
        if (btn.is('.dropdown-cur')) {
            btn.removeClass('dropdown-cur');
            qrcode.animate({top:50, opacity:0}, 200);

            if (isIE8) {
                qrcode.find('i').animate({opacity:0}, 200);
            }
        }
        else {
            btn.addClass('dropdown-cur');
            qrcode.animate({top:80, opacity:1}, 200);

            if (isIE8) {
                qrcode.find('i').animate({opacity:1}, 200);
            }
        }
...
/* 任性付 */
iFourth.renxingfu = function() {
    $('.renxf-item').click(function() {
        var li = $(this);
        ...
        if (li.is('.renxf-item-mianxi') && !li.is('.current')) {
            $('.renxf-btn .btn-mianxi').show();
            $('.renxf-btn .btn-fenqi').hide();

            if (window.Renxf) {
                Renxf.buttonId = 'rxfmx';
                Renxf.buttonClass = 'btn-mianxi';
            }
        }
        else {
            $('.renxf-btn .btn-mianxi').hide();
            $('.renxf-btn .btn-fenqi').show();

            if (window.Renxf) {
                Renxf.buttonId = 'rxffq';
